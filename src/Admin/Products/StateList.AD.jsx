...
const StateList = () => {
  ...
  const [newState, setNewState] = useState({ StateName: "" });
  ...
  const fetchStateList = async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error("Lỗi khi tải dữ liệu trạng thái");
      }
      ...
      setError(error.message || "Không thể tải dữ liệu trạng thái");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`${apiUrl}/deleteStateByID/${id}`, {
        method: 'DELETE',
      });

      if (!res.ok) {
        throw new Error("Không thể xóa trạng thái");
      }

      setStateList(prevList => prevList.filter(item => item._id !== id));
    } catch (error) {
      alert(error.message);
    } finally {
      setDeletingId(null);
  ...
  const handleAddState = async (e) => {
    ...
    setIsAdding(true);
    try {
      const res = await fetch(`${apiUrl}/create-state`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        throw new Error("Không thể thêm trạng thái");
      }

      const data = await res.json();
      setStateList(prevList => [...prevList, data.state]);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 to-gray-800">
      <div className="container mx-auto px-4 py-8">
        ...
        {!loading && !error && stateList.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[400px]">
            ...
            <p className="text-2xl text-gray-400">Không có trạng thái nào.</p>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-white">Thêm trạng thái mới</h3>
              <button
            ...
            <form onSubmit={handleAddState}>
              <div className="mb-6">
                <label className="block text-gray-300 mb-2">Tên trạng thái</label>
                <input
                  type="text"
                  value={newState.StateName}
                  onChange={(e) => setNewState({ StateName: e.target.value })}
