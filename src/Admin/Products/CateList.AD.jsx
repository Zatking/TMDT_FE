...
const CateList = () => {
  ...
  const [newCategory, setNewCategory] = useState({ CateName: "" });
  const [isAdding, setIsAdding] = useState(false);

  const fetchCateList = async () => {
    try {
      const res = await fetch(apiUrl + "/get-category");
      ...
      setError(error.message || "Không thể tải dữ liệu danh mục");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`${apiUrl}/deleteCateByID/${id}`, {
        method: 'DELETE',
      });
      ...
      setCateList(prevList => prevList.filter(item => item._id !== id));
    } catch (error) {
      alert(error.message);
    } finally {
      setDeletingId(null);
  ...
  const handleAddCategory = async (e) => {
    ...
    setIsAdding(true);
    try {
      const res = await fetch(`${apiUrl}/create-category`, {
        method: 'POST',
        headers: {
      ...
      const data = await res.json();
      setCateList(prevList => [...prevList, data.category]);
      setIsModalOpen(false);
      setNewCategory({ CateName: "" });
    } catch (error) {
      alert(error.message);
    } finally {
      setIsAdding(false);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-gray-900 to-gray-800">
      <div className="container mx-auto px-4 py-8">
        ...
        {!loading && !error && cateList.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[400px]">
            ...
            <p className="text-2xl text-gray-400">Không có danh mục nào.</p>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-xl p-8 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-white">Thêm danh mục mới</h3>
              <button
            ...
            <form onSubmit={handleAddCategory}>
              <div className="mb-6">
                <label className="block text-gray-300 mb-2">Tên danh mục</label>
                <input
                  type="text"
                  value={newCategory.CateName}
                  onChange={(e) => setNewCategory({ CateName: e.target.value })}
